...
'9 - Média do Saldo Acumulado' + '10 - Consultar por índice' + "11 - Alterar" +
'0 - Sair\n');
    opcao = input("Opção:");
...
function totalizarQtdContas(): number{
    return b.calcularQtdContas();
}


function totalizarQtdSaldo(): number{
    return b.calcularDinheiroDoBanco();
}


function calcularMediaSaldo(): number{
    return b.calcularMediaDoDinheiro();
}


function consultarPorIndice(): number{
    let numero: string = input('Digite o número da conta deseja descobrir o índice:');
    
    return b.consultarPorIndice(numero);
}
